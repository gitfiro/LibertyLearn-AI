...
const VALID_STATES_SET = new Set(US_STATES.map(s => s.toLowerCase()));
...
const FindAttorney: React.FC = () => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    if (formData.state.trim()) {
        ...
        if (!VALID_STATES_SET.has(stateInput)) {
            setError("Please enter a valid US State (e.g., 'VA' or 'Virginia').");
            return;
        }
    }
